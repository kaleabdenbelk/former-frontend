...
export function ProjectOverview() {
  ...
  const active = data.collections.filter((c) => c.status !== "ARCHIVED");
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <section className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <Button variant="ghost" size="sm" nativeButton={false} render={<Link href={`/projects/${projectId}/collections`} />}>
              View all
            </Button>
          </div>
          ...
          {recent.length === 0 ? (
            ...
            <ul className="divide-y divide-border overflow-hidden rounded-md border border-border">
              {recent.map((submission) => {
                ...
                const first = collection?.fields.find((f) => submission.data[f.name]);
                return (
                  <li key={submission.id}>
                    <Link
                      href={`/projects/${projectId}/collections/${submission.collectionId}`}
                      className="flex items-center justify-between gap-3 px-3 py-2.5 text-sm transition-colors hover:bg-accent"
                    >
                      <span className="min-w-0">
                        <span className="block truncate">
                          {first ? String(submission.data[first.name]) : "Submission"}
                        </span>
